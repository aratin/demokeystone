...
var Analyticalportal = keystone.list('Analyticalportal');
...
exports = module.exports = function(req, res) {
	...
	// Init locals
		locals.section = 'analyticalportal';
		locals.data = [];

	// Load the current Analyticalportal
	view.on('init', function(next) {
		var q = Analyticalportal.model.find();
		q.exec(function(err, results) {
			locals.data.analyticalportals = results;
			next(err);
		});
	});
