...
var Datawarehouse = keystone.list('Datawarehouse');
...
exports = module.exports = function(req, res) {
	...
	// Init locals
		locals.section = 'datawarehouse';
		locals.data = [];

	// Load the current Datawarehouse
	view.on('init', function(next) {
		var q = Datawarehouse.model.find();
		q.exec(function(err, results) {
			locals.data.datawarehouses = results;
			next(err);
		});
	});
