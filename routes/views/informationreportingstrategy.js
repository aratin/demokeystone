...
var Informationreportingstrategy = keystone.list('Informationreportingstrategy');
...
exports = module.exports = function(req, res) {
	...
	// Init locals
		locals.section = 'informationreportingstrategy';
		locals.data = [];

	// Load the current Informationreportingstrategy
	view.on('init', function(next) {
		var q = Informationreportingstrategy.model.find();
		q.exec(function(err, results) {
			locals.data.informationreportingstrategys = results;
			next(err);
		});
	});
	...
	// Render the view
	view.render('informationreportingstrategy');
}